...
class PiecesTab(Tab):
    ...
    def __update_callback(self, (is_fin, num_pieces, pieces, curdl)):
        ...
        for i,p in enumerate(pieces):
            if p:
                # todo set based on prio
                self._ms.setSquareColor(i,9)
            elif (cdli != -1 and i == curdl[cdli]):
                self._ms.setSquareColor(i,8)
                cdli += 1
                if cdli >= cdll:
                    cdli = -1
            else:
                try:
                    prio = self._prios[i]
                    # if prio > 0 and prio <= 7: 
                    self._ms.setSquareColor(i,prio)
                except KeyError:
                    pass

    def __update_prios(self, prios):
    ...
    def update(self):
        ...
        client.pieces.get_piece_priorities().addCallback(self.__update_prios)
